...
def run_algorithm(cfg: DictConfig):
    ...
    assert cfg['model'].MODEL_NAME in ['GD', 'FISTA', 'PnP']
    print(cfg['model'])
    if cfg['model'].MODEL_NAME == 'GD':
        model = GradientDescent(**set_up_data , **cfg['model'])
    elif cfg['model'].MODEL_NAME == 'FISTA':
        model = FISTA(**set_up_data , **cfg['model'])
    elif cfg['model'].MODEL_NAME == 'PnP':
        model = PnPMRI(**set_up_data , **cfg['model'])
    else:
